...
/*
    ...
    Written by Piotr Ulaszewski - December 2019, modified to display SMART in July 2021
            ...
            if (intstatus) {
                    ...
                    if (intstatus & AHCI_REG_PORT_IS_DHRS) {
                            ...
                            error = (ahci_port_read_dword (portnr, AHCI_REG_PORT_TFD) >> 8) & 0xFF;
                                                        ...
                                                        if (error) break;
                                                        else if (cmd_hdr->prdbc == length) break;
                    }
    ...
    while (i < 32) {
        ...
        if ((temp >> i) & 1) {
            ...
            if (type == 1) {
                ...
                // set to TRUE if drive supports 48bit LBA via direct access query (word 83 bit 10 if Identify Device result buffer)
                if (driveinfo->wCommandSetSupported2 & 1024) drive[total_drives].lba_48bit = TRUE;
                                else drive[total_drives].lba_48bit = FALSE;
...
BOOL ahci_start_disk_system (DISKDRIVE *sdrive)
{
    ...
    // AHCI access for SATA drive
    ahci_pci.active_port = sdrive->ahci_port;
    ahci_pci.active_port_bit = 1 << sdrive->ahci_port;

    type = ahci_port_check_type (ahci_pci.active_port);
    if (type == 1) {
        // SATA drive - allocate and setup our AHCI structure
        ahci_port_alloc (ahci_pci.active_port);
        ahci_port_reset (ahci_pci.active_port);
        if (ahci_port_setup (ahci_pci.active_port) == FALSE) return FALSE;
    }
...
void ahci_reset_disk_system (DISKDRIVE *sdrive)
{
    ...
    // AHCI access for SATA drive - stop & free our AHCI structure
    ahci_port_stop (ahci_pci.active_port);
    ahci_port_free (ahci_pci.active_port);
}
...
BOOL ahci_get_SMART_data (SMARTDATA *smartdata, DISKDRIVE *sdrive)
{
    ...
    // clear the 512 bytes buffer and set a temporary pointer to SMART structure
    BYTE *smartptr = (BYTE *)buff;
    memset ((BYTE *)buff, 0, 512);
    ...
    // count = 1, direction = 1
    if (!ahci_send_command (SMART_CMD, READ_ATTRIBUTES, 1, 1, sdrive, (BYTE *)buff)) return FALSE;

    // fill structure with SMART data
    smartptr += 2;   // skip smart revision

    // max of 30 smart attributes in 512 bytes of data
    for (i = 0; i < 30; i++) {
        smartdata->smartentry[i].attrib = (BYTE) *(smartptr);       // attribute ID
        smartdata->smartentry[i].flags = (WORD) *(smartptr + 1);    // flags - 2 bytes
        smartdata->smartentry[i].value = (BYTE) *(smartptr + 3);    // value
        smartdata->smartentry[i].worst = (BYTE) *(smartptr + 4);    // worst
        smartdata->smartentry[i].raw[5] = (BYTE) *(smartptr + 5);   // raw - 6 bytes
        smartdata->smartentry[i].raw[4] = (BYTE) *(smartptr + 6);   // raw - 6 bytes
        smartdata->smartentry[i].raw[3] = (BYTE) *(smartptr + 7);   // raw - 6 bytes
        smartdata->smartentry[i].raw[2] = (BYTE) *(smartptr + 8);   // raw - 6 bytes
        smartdata->smartentry[i].raw[1] = (BYTE) *(smartptr + 9);   // raw - 6 bytes
        smartdata->smartentry[i].raw[0] = (BYTE) *(smartptr + 10);  // raw - 6 bytes
        smartdata->smartentry[i].rawlword = (DWORD) ((smartdata->smartentry[i].raw[4] << 8) + (smartdata->smartentry[i].raw[5]));
        smartdata->smartentry[i].rawhword = (DWORD) ((smartdata->smartentry[i].raw[2] << 8) + (smartdata->smartentry[i].raw[3]));
        smartdata->smartentry[i].rawxword = (DWORD) ((smartdata->smartentry[i].raw[0] << 8) + (smartdata->smartentry[i].raw[1]));
        smartdata->smartentry[i].xxx = (BYTE) *(smartptr + 11);     // ? - 1 bytes
        smartptr += 12;
    }
    ...
    // count = 1, direction = 1
    if (!ahci_send_command (SMART_CMD, READ_THRESHOLDS, 1, 1, sdrive, (BYTE *)buff)) return FALSE;
    ...
    // max of 30 smart attributes in 512 bytes of data
    for (i = 0; i < 30; i++) {
        if (smartdata->smartentry[i].attrib != (BYTE) *(smartptr)) continue;    // compare attribute ID
        smartdata->smartentry[i].threshold = (BYTE) *(smartptr + 1);            // threshold

        // fill attribute names
        switch (smartdata->smartentry[i].attrib) {
            case 1:
                strcpy (smartdata->smartentry[i].name, "Raw read error rate");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 2:
                strcpy (smartdata->smartentry[i].name, "Throughput performance");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 3:
                strcpy (smartdata->smartentry[i].name, "Spinup time");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 4:
                strcpy (smartdata->smartentry[i].name, "Start/Stop count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 5:
                // special case for this attribute
                strcpy (smartdata->smartentry[i].name, "Reallocated sector count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 6:
                strcpy (smartdata->smartentry[i].name, "Read channel margin");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 7:
                strcpy (smartdata->smartentry[i].name, "Seek error rate");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 8:
                strcpy (smartdata->smartentry[i].name, "Seek time performance");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 9:
                strcpy (smartdata->smartentry[i].name, "Power-on hours count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 10:
                strcpy (smartdata->smartentry[i].name, "Spinup retry count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 11:
                strcpy (smartdata->smartentry[i].name, "Calibration retry count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 12:
                strcpy (smartdata->smartentry[i].name, "Power cycle count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 13:
                strcpy (smartdata->smartentry[i].name, "Soft read error rate");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                ...
                // extra attributes
            case 22:
                strcpy (smartdata->smartentry[i].name, "Current Helium Level");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 170:
                strcpy (smartdata->smartentry[i].name, "Available Reserved Space");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 171:
                strcpy (smartdata->smartentry[i].name, "SSD Program Fail Count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 172:
                strcpy (smartdata->smartentry[i].name, "SSD Erase Fail Count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 173:
                strcpy (smartdata->smartentry[i].name, "SSD Wear Leveling Count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 174:
                strcpy (smartdata->smartentry[i].name, "Unexpected power loss count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 175:
                strcpy (smartdata->smartentry[i].name, "Power Loss Protection Fail");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 177:
                strcpy (smartdata->smartentry[i].name, "Wear Range Delta");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 179:
                strcpy (smartdata->smartentry[i].name, "Used Reserved Block Count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 180:
                strcpy (smartdata->smartentry[i].name, "Unused Reserved Block Count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 181:
                strcpy (smartdata->smartentry[i].name, "Program Fail Count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 182:
                strcpy (smartdata->smartentry[i].name, "Erase Fail Count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 183:
                strcpy (smartdata->smartentry[i].name, "Runtime Bad Block");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 184:
                strcpy (smartdata->smartentry[i].name, "End-to-End error");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 185:
                strcpy (smartdata->smartentry[i].name, "Head Stability");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 186:
                strcpy (smartdata->smartentry[i].name, "Induced Op-Vibration");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 187:
                strcpy (smartdata->smartentry[i].name, "Uncorrectable Errors");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 188:
                strcpy (smartdata->smartentry[i].name, "Command Timeout");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 189:
                strcpy (smartdata->smartentry[i].name, "High Fly Writes");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 190:
                strcpy (smartdata->smartentry[i].name, "Airflow Temperature");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
            ...
            case 191:
                strcpy (smartdata->smartentry[i].name, "G-sense error rate");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 192:
                strcpy (smartdata->smartentry[i].name, "Power-off retract count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 193:
                strcpy (smartdata->smartentry[i].name, "Load/Unload cycle count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 194:
                strcpy (smartdata->smartentry[i].name, "HDA temperature");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 195:
                strcpy (smartdata->smartentry[i].name, "Hardware ECC recovered");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
            ...
            case 196:
                // special case for this attribute
                strcpy (smartdata->smartentry[i].name, "Reallocation events count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 197:
                strcpy (smartdata->smartentry[i].name, "Current pending sector count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 198:
                strcpy (smartdata->smartentry[i].name, "Offline scan uncorrectable");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 199:
                strcpy (smartdata->smartentry[i].name, "UDMA CRC error count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 200:
                strcpy (smartdata->smartentry[i].name, "Write error rate");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
            ...
            case 201:
                ...
                break;
            case 202:
                strcpy (smartdata->smartentry[i].name, "Data Address Mark errors");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 203:
                strcpy (smartdata->smartentry[i].name, "Run out cancel");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 204:
                strcpy (smartdata->smartentry[i].name, "Soft ECC correction");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 205:
                strcpy (smartdata->smartentry[i].name, "Thermal asperity rate");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 206:
                strcpy (smartdata->smartentry[i].name, "Flying height");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 207:
                strcpy (smartdata->smartentry[i].name, "Spin high current");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 208:
                strcpy (smartdata->smartentry[i].name, "Spin buzz");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 209:
                strcpy (smartdata->smartentry[i].name, "Offline seek performance");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 210:
                strcpy (smartdata->smartentry[i].name, "Vibration During Write");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
            ...
            case 211:
                ...
                break;
            case 212:
                strcpy (smartdata->smartentry[i].name, "Shock During Write");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 220:
                strcpy (smartdata->smartentry[i].name, "Disk shift");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
            ...
            case 221:
                ...
                break;
            case 222:
                strcpy (smartdata->smartentry[i].name, "Loaded hours");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 223:
                strcpy (smartdata->smartentry[i].name, "Load/unload retry count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 224:
                strcpy (smartdata->smartentry[i].name, "Load friction");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
            ...
            case 225:
                ...
                break;
            case 226:
                strcpy (smartdata->smartentry[i].name, "Load-in time");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 227:
                strcpy (smartdata->smartentry[i].name, "Torque amplification count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
            ...
            case 230:
                strcpy (smartdata->smartentry[i].name, "Drive Life Protection Status");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 231:
                strcpy (smartdata->smartentry[i].name, "SSD Life Left");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 232:
                strcpy (smartdata->smartentry[i].name, "Endurance Remaining");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 233:
                strcpy (smartdata->smartentry[i].name, "Power-On Hours");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 234:
                strcpy (smartdata->smartentry[i].name, "Avg and Max erase count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 235:
                strcpy (smartdata->smartentry[i].name, "Good and System block Count");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
            ...
            case 240:
                strcpy (smartdata->smartentry[i].name, "Head flying hours");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 241:
                strcpy (smartdata->smartentry[i].name, "Total LBAs Written");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 242:
                strcpy (smartdata->smartentry[i].name, "Total LBAs Read");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 249:
                strcpy (smartdata->smartentry[i].name, "Total NAND Writes");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
            ...
            case 250:
                strcpy (smartdata->smartentry[i].name, "Read error retry rate");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 251:
                strcpy (smartdata->smartentry[i].name, "Minimum Spares Remaining");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 252:
                strcpy (smartdata->smartentry[i].name, "Newly Added Bad Flash Block");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
                break;
            case 254:
                strcpy (smartdata->smartentry[i].name, "Free Fall Protection");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
            ...
            default:
                strcpy (smartdata->smartentry[i].name, "Vendor-specific");
                if (smartdata->smartentry[i].value > (smartdata->smartentry[i].threshold + 20)) smartdata->smartentry[i].creport = 0;
                else if (smartdata->smartentry[i].value > smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 1;
                else if (smartdata->smartentry[i].value == smartdata->smartentry[i].threshold) smartdata->smartentry[i].creport = 2;
                else smartdata->smartentry[i].creport = 3;
